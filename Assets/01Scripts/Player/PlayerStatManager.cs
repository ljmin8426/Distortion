...
public class PlayerStatManager : Singleton<PlayerStatManager>
{
    ...
    public static event StatsChange OnChangeAttackPower;
    public static event StatsChange OnChangeMoveSpeed;
    ...
    private IEnumerator RecoverStats()
    {
        ...
        while (true)
        {
            if (currentHP < MaxHP)
            {
                currentHP = Mathf.Min(currentHP + playerData.hpRegenRate * interval, MaxHP);
                OnHpChange?.Invoke(currentHP, MaxHP);
            }

            if (currentEP < MaxEP)
            {
                currentEP = Mathf.Min(currentEP + playerData.epRegenRate * interval, MaxEP);
    ...
    private void InitializeStats()
    {
        ...
        maxHP = playerData.baseMaxHP;
        maxEP = playerData.baseMaxEP;
        attackPower = playerData.baseAttack;
        ...
        recoverCoroutine = StartCoroutine(RecoverStats());
    }

    public void ApplyLevelUpGrowth()
    {
        level++;
        maxHP *= playerData.hpGrowth;
        maxEP *= playerData.epGrowth;
        attackPower *= playerData.atkGrowth;
        currentHP = MaxHP;
        currentEP = MaxEP;
